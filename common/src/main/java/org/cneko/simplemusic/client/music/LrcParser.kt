...
class LrcParser(val file: String){
    ...
    fun parseTime(time: String): Long {
        ...
        if (parts.size == 2) {
            ...
            val seconds = secondsAndMillis[0].toIntOrNull() ?: 0
            val millis = if (secondsAndMillis.size == 2) {
                (secondsAndMillis[1] + "000").substring(0, 3).toIntOrNull() ?: 0
            } else {
                0
            }

            milliseconds = (minutes * 60 * 1000 + seconds * 1000 + millis).toLong()
        }
